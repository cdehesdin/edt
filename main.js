...
class Creneau {
    ...
    toMin(h){ return h.split(":").map(Number).reduce((h,m)=>h*60+m); }
...
class Noeud {
    ...
    intersect(c){ return this.creneaux.some(x => x.intersect(c)); }
}

class Arbre {
    constructor(){ this.racine = new Noeud(); }

    insert(c){ this._rec(this.racine, c); }
    ...
    toList(){ return this._list(this.racine,1,0); }
...
function afficherJour(jour, divJour, debut="08:00", fin="18:00"){
    ...
    items.forEach(r => {
        ...
        b.style.left   = `${left}%`;
        b.style.width  = `${w}%`;
        if(d["background-color"]) b.style.backgroundColor = d["background-color"];

        b.innerHTML = `<div>${d.nom}</div><div>${d.info1}</div><div>${d.info2}</div>`;
